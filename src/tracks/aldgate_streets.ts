...
// D major, 128 bpm, 32-bar form: A (trumpet theme, bars 1-8), A2 (theme
// sequenced up a third, borrowed iv colour, bars 9-16), B (relative B minor,
// strings take the tune after a deceptive cadence, bars 17-24), A' (full-cry
// restatement, bars 25-32). The world leitmotif (C E G A G E) appears
// transposed to D in the counter-strings at bar 31.
...
// ---- Counter: strings — pads + answers in the lead's gaps; melody in B;
// leitmotif (C E G A G E -> D F# A B A F#) at bar 31. 32 bars.
const COUNTER =
  'A:3 - - - - - - - ' +
  'A:3 - - - . F#:4 G:4 A:5 ' +
  'B:3 - - - - - - - ' +
...
// ---- Inner line: pizzicato strings — busy urban eighths, chord-aware. 32 bars.
const bD  = 'D:5 A:3 D4:5 A:3 F#:4 A:3 D4:5 A:3 ';
const bG  = 'G:5 B:3 D4:5 B:3 G:4 B:3 D4:5 B:3 ';
const bA  = 'A:5 C#4:3 E4:5 C#4:3 A:4 C#4:3 E4:5 C#4:3 ';
const bBm = 'B:5 D4:3 F#4:5 D4:3 B:4 D4:3 F#4:5 D4:3 ';
const bEm = 'E:5 G:3 B:4 G:3 E:4 G:3 B:4 G:3 ';
const bFsm = 'F#:5 A:3 C#4:5 A:3 F#:4 A:3 C#4:5 A:3 ';
const bGm = 'G:5 A#:3 D4:5 A#:3 G:4 A#:3 D4:5 A#:3 ';
const bEmA = 'E:5 G:3 B:4 G:3 A:5 C#4:3 E4:5 C#4:3 ';
// B-section ghosts (quieter texture under the string melody)
...
const PIZZ =
  bD + bD + bG + bA + bBm + bG + bEmA + bD +          // A
  bD + bFsm + bG + bA + bBm + bGm + bEm + bA +        // A2
  gBm + gFsm + gG + gEm + gBm + gFs7 + gG + gA +      // B (ghosted)
  bD + bD + bG + bA + bBm + bGm + bEmA + bD;          // A'

// ---- Bass: walking acoustic bass, runs at the phrase turns. 32 bars.
const BASS =
  'D:7 - F#:5 - A:6 - B:6 - ' +
  'D3:6 - C#3:5 - B:5 - A:5 - ' +
  'G:7 - B:5 - D3:6 - B:5 - ' +
...
// ---- Drums: bright city backbeat, 8-bar loop with a tom fill into each turn.
const DRUMS =
  'K:7 H:4 S:6 H:3 K:5 H:4 S:6 H:3 ' +
  'K:7 H:4 S:6 H:3 K:5 H:3 S:6 O:4 ' +
  'K:7 H:4 S:6 H:3 K:5 H:4 S:6 H:3 ' +
  'K:7 H:4 S:6 S:2 K:5 H:3 S:6 O:4 ' +
  'K:7 H:4 S:6 H:3 K:5 H:4 S:6 H:3 ' +
  'K:7 H:4 S:6 H:3 K:5 H:3 S:6 O:4 ' +
  'K:7 H:4 S:6 H:3 K:5 S:2 S:6 H:3 ' +
  'K:7 H:4 S:6 S:3 T:6 T:7 U:7 C:7 ';

export const track: Track = {
  name: 'Aldgate Streets', bpm: 128, loopBars: 32,
  channels: [
    { program: P.TRUMPET, gain: 0.2, octave: 4, wave: 'square', pan: 0.05,
      notes: seq(LEAD) },
    { program: P.STRINGS, gain: 0.13, octave: 4, wave: 'sawtooth', pan: -0.4,
      notes: seq(COUNTER) },
    { program: P.PIZZICATO, gain: 0.1, octave: 3, wave: 'triangle', pan: 0.4,
      notes: seq(PIZZ) },
    { program: P.ACOUSTIC_BASS, gain: 0.17, octave: 2, wave: 'triangle', pan: 0,
      notes: seq(BASS) },
    { program: 0, gain: 0.12, octave: 3, wave: 'square', pan: 0.1, drums: true,
      notes: seq(DRUMS) },
  ],
};
