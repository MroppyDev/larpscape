...
The cube era is survived by one screenshot on the office wall, labeled *the Unstruck Age*. Onward.`}],n=(e,t="0 0 24 24")=>`<svg viewBox="${t}" aria-hidden="true" focusable="false">${e}</svg>`,h={sword:n(`<path d="M4 20 L15 9 M13 7 l4 -4 l4 1 l1 4 l-4 4 Z M3 17 l4 4 M2.5 21.5 l2 -2"
     ...
     <path d="M4.5 19.5 L19.5 6.5" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" opacity=".85"/>`)},R={"Game Updates":h.sword,"Dev Blog":h.quill,Community:h.banner,Events:h.bell};function U(e){return e.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function k(e){return U(e).replace(/\*\*([^*]+)\*\*/g,"<strong>$1</strong>").replace(/\*([^*]+)\*/g,"<em>$1</em>").replace(/`([^`]+)`/g,"<code>$1</code>")}function j(e){const t=e.trim().split(/\n\s*\n/),a=[];for(const l of t){const i=l.split(`
`).map(r=>r.trim()).filter(Boolean);i.length>0&&i.every(r=>r.startsWith("- "))?a.push(`<ul>${i.map(r=>`<li>${k(r.slice(2))}</li>`).join("")}</ul>`):a.push(`<p>${k(i.join(" "))}</p>`)}return a.join("")}const x="https://play.larpscape.net",v="https://wiki.larpscape.net",T="https://github.com/MroppyDev/larpscape";function o(e){const t=document.getElementById(e);if(!t)throw new Error(`missing #${e}`);return t}function s(e){return e.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}const F={"rail-game-list":[{label:"Play Larpscape",href:x,icon:"sword"},{label:"News & updates",href:"#news",icon:"scroll"},{label:"Hiscores",href:"#hiscores",icon:"trophy"},{label:"New player guide",href:`${v}/guide/getting-started`,icon:"book",ext:!0}],"rail-community-list":[{label:"Forum",href:q,icon:"quill",ext:!0},{label:"The Larpscape Wiki",href:v,icon:"book",ext:!0},{label:"Guilds & trading",href:"#about",icon:"banner"},{label:"GitHub",href:T,icon:"code",ext:!0}],"rail-account-list":[{label:"Create account",href:"/register",icon:"quill"},{label:"Log in",href:"/login",icon:"key"},{label:"Support",href:`${T}/issues`,icon:"bell",ext:!0}]};for(const[e,t]of Object.entries(F))o(e).innerHTML=t.map(a=>`<li><a href="${a.href}"${a.ext?' class="ext"':""}>${h[a.icon]}<span>${s(a.label)}</span></a></li>`).join("");(async()=>{const e=await A();if(!e)return;const t=o("auth-strip");t.innerHTML=`
    ...
    </a>`);(async()=>{try{const e=await fetch("/api/stats/online");if(!e.ok)return;const t=await e.json(),a=typeof t.online=="number"?t.online:t.count;if(typeof a!="number"||!Number.isFinite(a))return;o("player-count").innerHTML=`There are currently <strong>${a.toLocaleString("en-US")}</strong> ${a===1?"person":"people"} playing in Cantorne.`}catch{}})();const p=5,f=[...G].sort((e,t)=>t.date.localeCompare(e.date)),C=e=>new Date(`${e}T00:00:00`).toLocaleDateString("en-US",{day:"numeric",month:"long",year:"numeric"});function L(e,t,a){const l=R[e.category]??h.scroll;return`
    ...
    </button>`}const S=o("news-list");S.innerHTML=f.slice(0,p).map((e,t)=>L(e,t,t===0)).join("");const w=o("news-more");f.length>p&&(w.hidden=!1,w.addEventListener("click",()=>{S.insertAdjacentHTML("beforeend",f.slice(p).map((e,t)=>L(e,p+t,!1)).join("")),w.hidden=!0}));const d=o("news-modal"),I=o("modal-band"),D=o("modal-meta"),O=o("modal-title"),W=o("modal-body");let g=null;function N(e){I.style.setProperty("--band",e.accent),D.innerHTML=`<span class="card-tag" style="--band:${e.accent}">${s(e.category)}</span><time datetime="${e.date}">${C(e.date)}</time>`,O.textContent=e.title,W.innerHTML=j(e.body),d.hidden=!1,document.body.classList.add("modal-open"),o("modal-close").focus()}function b(){d.hidden=!0,document.body.classList.remove("modal-open"),g==null||g.focus()}document.addEventListener("click",e=>{const t=e.target.closest(".news-card");t&&(g=t,N(f[Number(t.dataset.post)]))});o("modal-close").addEventListener("click",b);d.addEventListener("click",e=>{e.target===d&&b()});document.addEventListener("keydown",e=>{e.key==="Escape"&&!d.hidden&&b()});const V=[{icon:"skills",title:"24 skills, one of them is Gun",text:"Train everything from Mining to Magic to the Aldgate Gun Guild’s pride and joy. Classic XP curves, level-up jingles, and a hiscores plaque with your name’s shape on it."},{icon:"quest",title:"Quests & the Untuned Mine",text:"A four-chapter questline that opens a sealed wing nobody should have opened, plus a solo instanced dungeon with dodge-on-the-beat bosses and a speedrun plaque."},{icon:"economy",title:"A player economy & guilds",text:"Trade face to face, post offers at the Aldgate Exchange, or found a guild with ranks, /g chat, and a shared vault. The first vault full of resonant shards earns our respect."},{icon:"offnote",title:"The Offnote is listening",text:"Cantorne was sung into being, and one wrong note slipped into the final cadence. Every monster is a place where the song skipped — killing them is retuning the world."}];o("feature-grid").innerHTML=V.map(e=>`
  <article class="feature">
    <span class="feature-icon" aria-hidden="true">${P[e.icon]}</span>
    <div><h3>${s(e.title)}</h3><p>${s(e.text)}</p></div>
  </article>`).join("");const Z=["Attack","Hitpoints","Mining","Strength","Agility","Smithing","Defence","Herblore","Fishing","Ranged","Thieving","Cooking","Prayer","Crafting","Firemaking","Magic","Fletching","Woodcutting","Runecraft","Slayer","Farming","Construction","Hunter","Gun"],y=o("skill-select");y.innerHTML='<option value="overall">Overall</option>'+Z.map(e=>`<option value="${e}">${e}</option>`).join("");const u=o("hiscores-body"),M=new Map;let m=0;function z(){return $("Level",'<tr class="skel-row"><td><span class="skel" style="width:24px"></span></td><td><span class="skel" style="width:110px"></span></td><td><span class="skel" style="width:42px"></span></td><td><span class="skel" style="width:80px;margin-left:auto"></span></td></tr>'.repeat(5))}function $(e,t){return`
    <div class="hiscores-table-wrap">
      ...
      </tr>`}).join("")}async function E(e){const t=++m,a=M.get(e);if(a){u.innerHTML=a;return}u.innerHTML=z();try{const l=await fetch(`/api/hiscores?skill=${encodeURIComponent(e)}&limit=10`);if(!l.ok)throw new Error(String(l.status));const i=await l.json();if(t!==m)return;let r;!i.ranking||i.ranking.length===0?r='<p class="hiscores-note">No names on the plaque yet — be the first on the board.</p>':r=$(e==="overall"?"Total":"Level",_(e,i.ranking)),M.set(e,r),u.innerHTML=r}catch{if(t!==m)return;u.innerHTML='<p class="hiscores-note error">The surveyor’s plaque is unreadable right now — try again shortly.</p>'}}y.addEventListener("change",()=>void E(y.value));E("overall");const B=new IntersectionObserver(e=>{for(const t of e)t.isIntersecting&&(t.target.classList.add("in"),B.unobserve(t.target))},{threshold:.08});document.querySelectorAll(".reveal").forEach(e=>B.observe(e));
