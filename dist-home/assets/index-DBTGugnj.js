...
`).map(r=>r.trim()).filter(Boolean);i.length>0&&i.every(r=>r.startsWith("- "))?a.push(`<ul>${i.map(r=>`<li>${k(r.slice(2))}</li>`).join("")}</ul>`):a.push(`<p>${k(i.join(" "))}</p>`)}return a.join("")}const x="https://play.larpscape.net",v="https://wiki.larpscape.net",F="https://trade.larpscape.net",T="https://github.com/MroppyDev/larpscape";function o(e){const t=document.getElementById(e);if(!t)throw new Error(`missing #${e}`);return t}function s(e){return e.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}const I={"rail-game-list":[{label:"Play Larpscape",href:x,icon:"sword"},{label:"News & updates",href:"#news",icon:"scroll"},{label:"Hiscores",href:"#hiscores",icon:"trophy"},{label:"New player guide",href:`${v}/guide/getting-started`,icon:"book",ext:!0}],"rail-community-list":[{label:"Forum",href:q,icon:"quill",ext:!0},{label:"The Larpscape Wiki",href:v,icon:"book",ext:!0},{label:"The Aldgate Exchange",href:F,icon:"economy",ext:!0},{label:"Guilds & trading",href:"#about",icon:"banner"},{label:"GitHub",href:T,icon:"code",ext:!0}],"rail-account-list":[{label:"Create account",href:"/register",icon:"quill"},{label:"Log in",href:"/login",icon:"key"},{label:"Support",href:`${T}/issues`,icon:"bell",ext:!0}]};for(const[e,t]of Object.entries(I))o(e).innerHTML=t.map(a=>`<li><a href="${a.href}"${a.ext?' class="ext"':""}>${h[a.icon]}<span>${s(a.label)}</span></a></li>`).join("");(async()=>{const e=await A();if(!e)return;const t=o("auth-strip");t.innerHTML=`
  ...
  <article class="feature">
    ...
    </div>`}function Q(e,t){return t.map(a=>{const l="totalLevel"in a?a.totalLevel:a.level,i="totalXp"in a?a.totalXp:a.xp;return`<tr${a.rank<=3?` class="podium-${a.rank}"`:""}>
